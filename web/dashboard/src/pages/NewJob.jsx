...
import { createJob, uploadVideo, fetchCookiesStatus, uploadCookies, deleteCookies } from '../api'
...
function NewJob() {
  ...
  // Cookies (global, shared across jobs)
  ...
  useEffect(() => {
    fetchCookiesStatus()
      .then((d) => setCookiesExist(d.exists))
  ...
  const handleCookiesDelete = async () => {
    if (!window.confirm('Hapus cookies.txt dari server?')) return
    await deleteCookies()
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className="card" style={{ marginBottom: '16px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '16px' }}>
            <span style={{ fontSize: '13px', color: cookiesExist ? 'var(--success)' : 'gray' }}>
              {cookiesExist ? '✅ Cookies aktif' : '🍪 Belum ada cookies'}
            </span>
            <input
              ref={cookiesFileRef}
              type="file"
            ...
            >
              {cookiesUploading ? 'Mengupload...' : '📁 Upload cookies.txt'}
            </button>
            {cookiesExist && (
              <button type="button" className="btn btn-ghost btn-sm" onClick={handleCookiesDelete}>
                🗑️ Hapus
              </button>
            )}
          </div>
